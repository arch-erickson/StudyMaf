...
function render() {
  ...
  root.querySelector('#signin-email-form').onsubmit = async function (event) { event.preventDefault(); const button = event.currentTarget.querySelector('button'); const address = root.querySelector('#signin-email').value.trim(); button.disabled = true; button.textContent = 'Sending…'; error(''); try { await Auth.sendEmailCode(address); codeScreen(address); } catch (reason) { error(reason.message); button.disabled = false; button.textContent = 'Send verification code →'; } };
}
render();
