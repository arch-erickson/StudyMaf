...
// Control is a dedicated administrator surface. A previous visit to the
// student or professor sign-in page may have left that workspace preference in
// localStorage; never let it block a valid administrator session here.
Auth.setWorkspace('admin');
...
document.querySelector('#google').onclick = function () { try { Auth.setWorkspace('admin'); Auth.startGoogle(location.href); } catch (reason) { showError(reason.message); } };
form.onsubmit = async function (event) { event.preventDefault(); const button = form.querySelector('button'); button.disabled = true; showError(''); try { const address = document.querySelector('#email').value.trim(); Auth.setWorkspace('admin'); await Auth.sendEmailCode(address, location.origin + location.pathname); verifyScreen(address); } catch (reason) { showError(reason.message); button.disabled = false; } };
