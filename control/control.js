...
import { createAccountUI } from '../js/account-ui.js?v=5';
...
const accountInitials = document.querySelector('#control-initials');
function showError(message) { error.textContent = message || ''; error.hidden = !message; }
function closeModal() { host.hidden = true; host.innerHTML = ''; }
function modal(html, options) { host.hidden = false; host.innerHTML = '<div class="control-modal-backdrop"><section class="control-modal-card' + (options && options.wide ? ' wide' : '') + '">' + html + '</section></div>'; host.querySelectorAll('[data-close]').forEach(function (button) { button.onclick = closeModal; }); return host.querySelector('.control-modal-card'); }
...
function renderPortal() { const account = Auth.getAccount(); if (!account || account.role !== 'admin') return deny(account ? 'This account is not approved for StudyMAF Control.' : 'Sign in with an administrator account.'); card.hidden = true; bottom.hidden = true; app.hidden = false; accountEmail.textContent = account.user.name || account.user.email; accountInitials.textContent = String(account.user.name || account.user.email || 'A').split(/\s+|@/).map(function (part) { return part.charAt(0); }).join('').slice(0, 2).toUpperCase(); accountBar.hidden = false; shell.classList.add('portal'); portal.renderAdmin(app); }
