...
(() => {
  ...
  d.title("Constant velocity");
  d.line([24, 188], [476, 188], { color: d.palette.ink, width: 1.6 });
...
(() => {
  ...
  d.title("Field between two charges");
  ...
  save("02-electrostatics", "authored charges (red/blue) + field arrows + ± symbols + dodged labels", d);
})();
...
(() => {
  ...
  for (const x of [175, 305]) { d.content.push(`<circle cx="${x}" cy="130" r="2.4" fill="${d.palette.ink}"/>`); d.boxes.push({ x: x - 5, y: 125, w: 10, h: 10 }); d.text("F", x, 150, { anchor: "middle", size: 12, math: true, color: d.palette.ink, avoid: true }); }
...
// gallery
let html = `<h1 style="font-family:system-ui;margin:0 0 4px">DGE — example diagrams</h1><p style="font-family:system-ui;color:#667;margin:0 0 18px">Composed from the visual library: objects recolored to a palette, annotations/symbols/text placed to avoid obstruction, consistent annotation colors (generic arrow one muted neutral; semantic vectors keep their role color; charge signs red/blue), and muted object palettes.</p><div style="display:flex;flex-wrap:wrap;gap:22px">`;
